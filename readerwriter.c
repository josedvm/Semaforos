...
#define SEMLEC "/mysemaphoreasasasas"
#define SEMESC "/mysemaphore223232"
int contador;
int contadorL;
int contadorE;
int numreaders;
...
int main() {
s1 = sem_open(SEMLEC, O_CREAT, S_IRUSR | S_IWUSR, 1); //creacion del semaforo
s2 = sem_open(SEMESC, O_CREAT, S_IRUSR | S_IWUSR, 1); //creacion del semaforo
...
void  INThandler(int sig)
{
     ...
     c = getchar();
     if (c == 'y' || c == 'Y') {
 		sem_close(s1); sem_unlink(SEMLEC);
 		sem_close(s2); sem_unlink(SEMESC);
		exit(0);
     } else
          	signal(SIGINT, INThandler);
